...
# the dependence on Makefile.PL is necessary because if it happens to be newer than the XS sources or object files,
# the latter are mercilessly deleted.
${GlueObjects} ${CallableGlueObjects} ${XXSObjects} $(patsubst %,${PerlExtDir}/%.cc,${XXSModules}) : ${SourceDir}/perl/Makefile.PL | compile-xs
...
ifneq (${LDcallableFlags},none)
  ...
  ${CallableLib} : ${SharedObjects} ${CallableSharedObjects} ${GlueObjects} ${CallableGlueObjects} ${XXSObjects} $(patsubst %,${SourceDir}/perl/%.xs,${XSModules})
	${CXX} ${LDcallableFlags} ${LDsonameFlag}${CallableSoName} -o $@ ${SharedObjects} ${CallableSharedObjects} ${GlueObjects} ${CallableGlueObjects} ${XXSObjects} ${XSObjects} ${LDFLAGS} ${PERLccdlflags} -lmpfr -lgmp -lpthread ${LIBXML2_LIBS} ${LIBS} -L${PERLarchlib}/CORE -lperl
...
compile-xs:
	@mkdir -p ${PerlExtDir}
	cd ${PerlExtDir}; \
	${PERL} ${SourceDir}/perl/Makefile.PL \
		TOP=${ProjectTop} SourceDir=${SourceDir}/perl CxxOpt="${CXXOPT}" CxxDebug="${CXXDEBUG}" \
		libxml2Cflags="${LIBXML2_CFLAGS}" libxml2Libs="${LIBXML2_LIBS}"
	@$(MAKE) -C ${PerlExtDir} compile

compile : ${CoreLib} ${CallableLib}
