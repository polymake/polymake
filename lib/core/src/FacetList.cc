...
{
   if ((c->lex.prev->lex.next=below) != NULL)
...
void facet::unlink_cells(chunk_allocator& al)
{
   ...
   for (;;) {
      ...
      if ((below->lex.next=cur->lex.next) != NULL)
   ...
   : vertex(l.vertex)
{
   ...
   for (cell* orig=l.first_col;  orig != NULL;  orig=orig->col.next) {
      if (orig->lex.next != NULL) {
...
bool vertex_list::inserter::push(vertex_list& column, cell* newc)
{
   ...
   for (;;) {
      ...
      if ((first_old=last_old->lex.next) == NULL) {
...
bool vertex_list::inserter::new_facet_ended()
{
   ...
   if (last_old != NULL && last_old->facet.next != NULL) {
      finalize();
      return true;
   }
   return false;
...
bool Table::sanity_check() const
{
   ...
   for (const_iterator f=begin(), fe=end();  f != fe;  ++f, ++n_f) {
      ...
      for (auto e=entire(*f); !e.at_end(); ++e, ++n_e) {
         ...
         if (e->col.prev == NULL)
            _show_facet(cerr, *f, shown) << "col.prev[" << i << "]=NULL" << endl;
         else if (e->col.prev->col.next != e.operator->())
            _show_facet(cerr, *f, shown) << "col.prev[" << i << "] mismatch" << endl;
         else if (e->col.next != NULL && e->col.next->col.prev != e.operator->())
            _show_facet(cerr, *f, shown) << "col.next[" << i << "] mismatch" << endl;

         if (e->lex.prev != NULL && e->lex.prev->lex.next != e.operator->())
            _show_facet(cerr, *f, shown) << "lex.prev[" << i << "] mismatch" << endl;
         else if (e->lex.next && e->lex.next->lex.prev != e.operator->())
            _show_facet(cerr, *f, shown) << "lex.next[" << i << "] mismatch" << endl;

         if (e->lex.prev != NULL) {
            if (lex_prev_seen)
               _show_facet(cerr, *f, shown) << "lex.prev[" << i << "] multiple" << endl;
            else
               lex_prev_seen=true;
         }
         if (e->lex.next != NULL) {
            if (!lex_prev_seen) {
               _show_facet(cerr, *f, shown) << "lex.next[" << i << "] before lex.prev" << endl;
            } else {
