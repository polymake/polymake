...
use Polymake::Struct (
   ...
   '$changed',                          # BigObject changed since last load() or save() (only for top-level objects)
...
# for compatibility with PropertyValue
sub value { $_[0] }
...
use Polymake::Struct (
   [ new => '$@' ],
   [ '$content_end' => '$#{#1->contents}' ],    # length of BigObject->contents at the transaction start
...
# [ Property, BigObject ]           : multiple subobject added temporarily
# [ Property, [ list ] ]         : single subobject with its own list of temporaries
# [ Property, BigObject, [ list ] ] : multiple subobject with its own list of temporaries
...
sub merge_temporaries {
   my ($dst, $src)=@_;
   if (defined(my $deleted_index = shift @$src)) {
      assign_min($dst->[0], $deleted_index);
   }
...
sub commit {
   my ($self, $object, $parent_trans)=@_;
   until (Scheduler::resolve_initial_request($object, [ [ BigObjectType::init_pseudo_prop() ], requests_for_subobjects($self, $object) ])) {
      my $err=$@;
      if (defined $parent_trans) {
         $parent_trans->rollback($object->parent);
      } else {
         &rollback;
         if (instanceof BigObject::Replacement($err)) {
            undef $@;
            %{$self->subobjects}=();
...
sub requests_for_subobjects {
   ...
   map {
      ...
      ( [ @descend, $prop, BigObjectType::init_pseudo_prop() ],
...
# by the prototype (BigObjectType) constructor.
...
sub cast_me {
   ...
   my $need_commit = open_or_check_transaction($self);
...
sub set_as_default_now {
   ...
   select_multi_instance($self, $Scope->locals);
}
...
sub lookup_descending_path {
   ...
   if (($obj, $prop) = descend_partially($self, \@existing_path, $path)) {
      if (defined($content_index = $obj->dictionary->{$prop->key}) &&
         ...
         if (!$for_planning   # nested scheduler runs should still see this as `undef'
               and
...
sub lookup_pv {         # 'request' => PropertyValue or BigObject or 0 or undef
...
# pure read-only lookup, no shortcuts
# return boolean
sub has_property {
   my ($self, $prop_name) = @_;
   if (defined(my $prop = $self->type->lookup_property($prop_name))) {
...
sub remove {
   ...
   if (@_ == 2 && is_object($_[1])) {
      ...
      $need_commit = open_or_check_transaction($self);
      ...
      my @to_remove = map {
         ...
         if ($enable_plausibility_checks && $from_production_rule) {
...
####################################################################################
sub prepare_for_schema_creation {
   my ($self) = @_;
   if (defined($self->parent)) {
...
sub give_pv {
   ...
   defined($pv)
   ? do {
        close_pending_transaction($self, $req_string);
...
sub give_list {
   my ($self, $req)=@_;
   my @requests = map { $self->type->encode_read_request($_) } is_array($req) ? @$req : split(/\s*,\s*/, $req);
...
####################################################################################
sub open_or_check_transaction {
   my ($self) = @_;
   if (defined($self->transaction)) {
      if (defined($self->transaction->rule)) {
         croak( (caller(1))[3], " not allowed within a rule" );
      }
   } else {
      begin_transaction($self);
      1
...
####################################################################################
sub close_pending_transaction {
   my ($self, $get_prop) = @_;
   if (defined($self->transaction)) {
      if (defined($self->transaction->rule)) {
         if (defined($get_prop)) {
            if ($self->transaction->in_schedule) {
               die "Infeasible\n";
            } else {
               croak( "rule ", $self->transaction->rule->header, " tried to read a non-existing property ", is_object($get_prop) ? $get_prop->name : $get_prop );
            }
         }
         croak( (caller(1))[3], " not allowed within a rule" );
      } else {
...
sub properties {
   ...
   $data[-1] .= "\n";
   foreach (sort $self->list_properties) {
...
sub get {
   ...
   unless (defined($content_index)) {
      close_pending_transaction($self, $prop);
...
sub remove_attachment {
   local interrupts(block);
   my ($self, $name) = @_;
   if (defined(delete $self->attachments->{$name})) {
      if (defined($self->transaction)) {
         if (defined($self->transaction->rule)) {
...
sub attach {
   ...
   if (is_object($data)) {
      ...
      } elsif (instanceof BigObjectType($type)) {
...
# if no BigObjectType in the type hierarchy has a specific suffix
...
sub diff {
   ...
   if (keys %options) {
      croak( "BigObject::diff - unrecognized option(s) ", join(", ", keys %options) );
   }
...
sub ensure_save_changes {
   my ($self) = @_;
   state $at_end = add AtEnd("BigObject",
      sub {
...
####################################################################################
package Polymake::Core::BigObject::Replacement;
