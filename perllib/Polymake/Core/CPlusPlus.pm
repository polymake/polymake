...
use Polymake::Struct (
   ...
   [ '$embedded_rules' => 'undef' ],   # CODE ref while rulefiles loading
...
sub pick_embedded_rules {
   ...
   if ($self->embedded_rules_cnt > 0) {
      $self->embedded_rules=do "c++:".$shared_mod->so_name;
      if ($@) {
...
sub add_auto_function {
   ...
   if (defined($auto_func->embedded)) {
      ...
      if ($flags & $func_is_method and $name eq 'operator') {
         ...
         undef $credit if $ext_code;  # will be displayed in $ext_code if needed
         $code= defined($returns) ? sub { $credit->display if $credit && $Verbose::credits > $credit->shown;
                                          unshift @_, $returns;
   ...
   if (defined $subst_for_temp_src) {
      ...
      push @{$self->duplicate_class_instances},
           map {
              ...
              $_->extension=$Application::extension;
              $_
...
sub define_constructors {
   ...
   Struct::pass_original_object(\&{$symtab->{construct}});
}
...
sub create_methods {
   ...
   if (exists $symtab->{construct}) {
      ...
      $proto->construct=define_constructors($proto->cppoptions, $proto->pkg, $symtab);
      ...
      if ($generic_opts->builtin) {
         ...
         # either there is no base class at all, or it is not instantiated:
         # descend through generic classes without own constructors
         my $opts=$proto->cppoptions;
         my $generic_pkg=*{$symtab->{ISA}}->[0];
         for (;;) {
            no strict 'refs';
            last if exists &{"$generic_pkg\::construct"};
            my $super_generic_pkg=${"$generic_pkg\::ISA"}[0];
            last unless defined($super_generic_pkg) && exists &{"$super_generic_pkg\::c++options"};
            $generic_pkg=$super_generic_pkg;
            $opts=&{"$super_generic_pkg\::c++options"}();
         }
         $proto->construct=define_constructors($opts, $generic_pkg);
      }
   } elsif (!can_inherit_constructors($proto->super)) {
