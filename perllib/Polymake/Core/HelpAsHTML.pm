...
sub write_possible_values {
   ...
   foreach my $value (@$values) {
      ...
      writeText($self, $value->[0]);
      $self->writer->endTag;
      $self->writer->startTag([ $xhtmlns, "dd" ]);
      writeText($self, $value->[1]);
      $self->writer->endTag;
   }
   $self->writer->endTag;
   $save_mode and $self->writer->setDataMode($save_mode);
...
sub function_options {
   ...
   foreach (@options) {
      writeTableRow($self, "$_->[1] =>", $_->[0], $_->[2]);
   }
}
...
sub function_return {
   ...
   if (defined $return) {
      ...
      writeTableRow($self, $return->[0], "", $return->[1] // "");
   }
