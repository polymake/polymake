...
namespace polymake { namespace tropical { 
   ...
   typedef std::pair< Matrix<Rational>, std::vector< Set<int> > > StarResult;
	...
	Integer computeFanMultiplicity(const Matrix<Rational> &xrays, const Matrix<Rational> &xlin,
         const std::vector<Set<int> > &xcones, const Vector<Integer> &xweights, const int xdim,
         const Matrix<Rational> &yrays, const Matrix<Rational> &ylin,
         const std::vector<Set<int> > &ycones, const Vector<Integer> &yweights, const int ydim) {
		Integer weight(0);
		solver<Rational> sv;
      ...
      std::vector<int> full_dimensional_yindex;
		for(auto xc = ensure( xcones, (pm::cons<pm::end_sensitive, pm::indexed>*)0).begin(); !xc.at_end(); ++xc) {
			for(auto yc = ensure( ycones, (pm::cons<pm::end_sensitive, pm::indexed>*)0).begin(); 
               !yc.at_end(); ++yc) {
				auto x_sub_rays = xrays.minor(*xc,All);
				auto y_sub_rays = (- yrays.minor(*yc,All));
		...
		do {
         ...
         for(auto fullcone = ensure(full_dimensional_cones,(pm::cons<pm::end_sensitive, pm::indexed>*)0).begin();
               !fullcone.at_end(); ++fullcone) {
				...
				if(is_interior) {
					Integer latticeIndex = lattice_index(
							lattice_basis_of_cone(
								xrays.minor(xcones[full_dimensional_xindex[fullcone.index()]],All),xlin,xdim,false) / 
							lattice_basis_of_cone(
								yrays.minor(ycones[full_dimensional_yindex[fullcone.index()]],All),ylin,ydim,false));
	...
	perl::ListReturn intersect_check_transversality(perl::Object X, perl::Object Y, bool ensure_transversality = false) {
      ...
      std::vector<Set<int> > ycontainers;
		...
		for(auto ic = ensure( rows(f.cones), (pm::cons<pm::end_sensitive, pm::indexed>*)0).begin();
            !ic.at_end(); ++ic) {
		...
		for(auto c = ensure(intercones, (pm::cons<pm::end_sensitive, pm::indexed>*)0).begin();
            !c.at_end(); ++c) {
			//Find interior point
         ...
         ystar_rays = stary.first; ystar_cones = stary.second;
			...
			Integer w = computeFanMultiplicity(
					xstar_rays, xlin_dehom, xstar_cones, xweights.slice(xcontainers[c.index()]), Xdim,
		...
		if(weight_zero_cones.size() == intercones.size()) {
			perl::ListReturn zeroResult;
			zeroResult << empty_cycle<Addition>(Xambi);
			zeroResult << false;
			return zeroResult;
