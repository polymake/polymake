...
namespace polymake { namespace tropical {
	...
	inline Matrix<Rational> reduce_rays(const Matrix<Rational> &m) {
		Set<int> vertex = far_and_nonfar_vertices(m).first;
		return tdehomog(m).minor(vertex, ~scalar2set(0));
	}
	...
	template <typename Addition>
		perl::ListReturn is_smooth(perl::Object f){
			...
			if(!g.CallPolymakeMethod("is_fan")) {
				R.is_smooth=0;
			}
			...
			if (R.is_smooth==1) {
				...
				perl::Object mapmorphism(perl::ObjectType::construct<Addition>("Morphism"));
	...
	result smooth_dim1(perl::Object f){
		...
		std::pair<Matrix<Integer>, SparseMatrix<Integer> > hnfresult =
			common::hermite_normal_form(Rays);
		Matrix<Integer> tfmatrix(hnfresult.second);

		//Compute the determinant of HNF
		Integer determinant=det(hnfresult.first);
