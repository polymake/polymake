   ...
   friend std::ostream& operator<< (std::ostream& os, const simplex_rep_iterator& sit) {
      ...
      for (int i=0; i<sit.d+1; ++i) {
         ...
         for (int j=0; j<sit.next_orbits[i].size(); ++j) {
            os << "{";
            for (Entire<Set<int> >::const_iterator ssit = entire(sit.next_orbits[i][j]); !ssit.at_end(); ++ssit) {
               os << *ssit << " ";
            }
            os << "}";
         }
...
protected:
   ...
   void reset_current_simplex(type2type<common::boost_dynamic_bitset>) {
      current_simplex.reset();
   }
