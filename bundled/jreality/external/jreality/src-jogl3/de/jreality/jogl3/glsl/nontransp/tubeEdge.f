...
uniform vec4 lineShader_polygonShader_ambientColor;
uniform float lineShader_polygonShader_ambientCoefficient;

uniform vec4 lineShader_polygonShader_specularColor;
uniform float lineShader_polygonShader_specularCoefficient;
uniform float lineShader_polygonShader_specularExponent;
...
void calculateLightInfluxGeneral(vec3 normal, int numDir, int numPoint, int numSpot, sampler2D lights){	
	...
	for(int i = 0; i < numDir; i++){
		...
		vec4 ambient = lineShader_polygonShader_ambientColor*col*intensity;
		lightInflux = lightInflux + lineShader_polygonShader_ambientCoefficient * ambient.xyz;
		...
		if(dott > 0){
			...
			vec4 specular =lineShader_polygonShader_specularColor*intensity*pow(spec, lineShader_polygonShader_specularExponent);
			
			vec4 new = lineShader_polygonShader_specularCoefficient*specular+lineShader_polygonShader_diffuseCoefficient*diffuse2;
	...
	for(int i = 0; i < numPoint; i++){
		...
		vec4 ambient = lineShader_polygonShader_ambientColor*col*intensity;
		lightInflux = lightInflux + atten*lineShader_polygonShader_ambientCoefficient * ambient.xyz;
	...
	for(int i = 0; i < numSpot; i++){
		...
		if(angle < coneAngles.x){
			...
			lightInflux = lightInflux + factor*atten*lineShader_polygonShader_ambientCoefficient * ambient.xyz;
...
void calculateGlobalLightInflux(vec3 normal){
	lightInflux = lightInflux + lineShader_polygonShader_ambientColor.xyz*lineShader_polygonShader_ambientCoefficient;
...
void main(void)
{
	...
	diffuse = lineShader_polygonShader_diffuseColor;
