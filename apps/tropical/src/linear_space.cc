...
namespace polymake { namespace tropical {
   ...
   template <typename Addition>
      perl::Object linear_space(perl::Object valuated_matroid) {
         ...
         Array<Set<int> > non_including_bases(n);
         ...
         IncidenceMatrix<> max_polys = fan::tight_span_from_incidence_with_excluded_faces(IncidenceMatrix<>(std::move(flacets)), Set<Set<int> >(non_including_bases),-1);
         ...
         perl::Object result(perl::ObjectType::construct<Addition>("Cycle"));
            ...
            result.take("MAXIMAL_POLYTOPES") << max_polys;
            result.take("LINEALITY_SPACE") << lin_space;
            result.take("WEIGHTS") << ones_vector<Integer>(max_polys.rows());
