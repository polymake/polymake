...
Array<perl::Object> construct_cones(const Array<Set<int> > & max_cones, const Matrix<Coord> & rays, const Matrix<Coord> & lin_space){
   perl::ObjectType cone_type=perl::ObjectType::construct<Coord>("Cone");

   int size = lin_space.rows() > 0 ? max_cones.size()+1 : max_cones.size();
   ...
   // if the fan does not have maximal cones, we still need something to intersect with
   if (lin_space.rows() > 0) {
      all_cones[max_cones.size()].create_new(cone_type);
      const Matrix<Coord> ns = null_space(lin_space);
      all_cones[max_cones.size()].take("INEQUALITIES")<< ns/(-ns);
   }
...
perl::Object common_refinement(perl::Object f1, perl::Object f2)
{
   ...
   const Matrix<Coord> lineality_space = (lineality_space1.cols() > 0 && lineality_space2.cols() > 0)?
      null_space(null_space(lineality_space1) / null_space(lineality_space2)) : Matrix<Coord>() ;
   ...
   */
   ...
   Array<perl::Object> all_cones1 = construct_cones(max_cones1, rays1, lineality_space1);
   Array<perl::Object> all_cones2 = construct_cones(max_cones2, rays2, lineality_space2);
   ...
   for (Entire<Array<perl::Object> >::iterator i1=entire(all_cones1); !i1.at_end(); ++i1) {
      for (Entire<Array<perl::Object> >::iterator i2=entire(all_cones2); !i2.at_end(); ++i2) {
         ...
         if (inters_dim==d || (!complete && inters_dim > 0)) {
            ...
            if (new_cone.size() > 0) new_max_cones.push_back(new_cone);
         }
      }
   }
