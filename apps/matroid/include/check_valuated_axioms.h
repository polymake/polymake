...
namespace polymake { namespace matroid {
  ...
  TropicalNumber<Addition,Scalar> find_valuation(const Array<Set<int> > &bases, const Vector<TropicalNumber<Addition,Scalar> > &valuation, const Set<int> &s) {
  ...
  template
  <typename Addition, typename Scalar>
  bool check_valuated_basis_axioms(const Array<Set<int> > &bases, const Vector<TropicalNumber<Addition, Scalar> > &valuation, perl::OptionSet options) {
    ...
    if(bases.size() == 0) {
	if(verbose) pm::cout << "There is no basis" << endl;
	return false;
    }
    
    //Convert to set for convenience
	...
	if(i != j) {
	  ...
	  for(Entire<Set<int> >::iterator u = entire(iwithoutj); !u.at_end(); u++) {
	    bool found_one = false;
	    for(Entire<Set<int> >::iterator v = entire(jwithouti); !v.at_end(); v++) {
	    ...
	    if(!found_one) {
	      if(verbose) pm::cout << "No basis exchange for bases " << i << " and " << j << " and element " << *u << endl;
	      return false;
	    }
  ...
  bool check_valuated_circuit_axioms(const Matrix<TropicalNumber<Addition,Scalar> > &valuations, perl::OptionSet options) {
	...
	if( TropicalNumber<Addition,Scalar>::zero() != valuations(r,i)) supp+= i;
      ...
      if(supp.size() == 0) {
	  if(verbose) pm::cout << "Circuit " << r << " is empty. " << endl;
	  return false;
      }
      supports |= supp;
	...
	for(int j = i+1; j < supports.dim(); j++) {
		...
		if(verbose) pm::cout << "Circuits " << i << " and " << j << " have contained supports." << endl;
		return false;
	    }
	}
    }
	...
	if(x != y) {
	  ...
	  for(Entire<Set<int> >::iterator u = entire(suppboth); !u.at_end(); u++) {
	    for(Entire<Set<int> >::iterator v = entire(supponlyx); !v.at_end(); v++) {
	      bool found_one = false;
...
// 		    ( valuations(y,*u) / valuations(x,*u)) * valuations.row(x);
...
// 		    (( xprime[*v] / valuations(z,*v)) * valuations.row(z));
		  //Check if Z' + X' + Y = X' + Y
	      ...
	      if(!found_one) {
		  if(verbose) {
		      pm::cout << "Circuit exchange axiom fails for circuits " << x << " and " << y << " with " << *u << " in common support and " << *v << " only in circuit " << x << endl;
		  }
		  return false;
	      }//END if none found
	    }//END iterate X\Y
