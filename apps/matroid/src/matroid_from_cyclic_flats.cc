...
perl::Object matroid_from_cyclic_flats(const Array<Set<int>>& Faces, Array<int> Ranks, const int& n_elements)
{
   graph::HasseDiagram LF;
   graph::HasseDiagram::_filler HD(LF,true);
   ...
   for (Entire<Set<Set<int>, CompareByRank>>::const_iterator it=entire(sorted_sets); !(it.at_end()); ++it) {
      int new_rank = RankMap[*it]; 
      for (; rank<new_rank; ++rank)
         HD.increase_dim();    
      HD.add_node(*it);
      ...
      Graph<Directed> indg = induced_subgraph(HD.graph(),incl_set_indices);
      for (Entire<Nodes<Graph<Directed>>>::const_iterator from_node=entire(nodes(indg)); !(from_node.at_end()); ++from_node)
         if (indg.out_degree(*from_node) == 0) 
            HD.add_edge(*from_node,HD.graph().nodes()-1);
   }
