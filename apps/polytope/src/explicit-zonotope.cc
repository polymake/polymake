...
perl::Object explicit_zonotope(const Matrix<E>& zones, perl::OptionSet options)
{
   ...
   const int dim = rows_are_points ? zones.cols() : zones.cols() + 1;
   Matrix<E> points(1, dim);    // one zero-filled row
   ...
   Z.take(rows_are_points ? "ZONOTOPE_INPUT_POINTS" : "ZONOTOPE_INPUT_VECTORS") << zones;
