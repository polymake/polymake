...
/* added (back?) by db, March 29, 2006 */
int lrs_init_quiet(FILE *ifp, FILE *ofp){
  if (!lrs_mp_init (ZERO, ifp, ofp))  /* initialize arithmetic */
    return FALSE;
#ifdef SIGNALS
...
/* Used also with lponly flag                                          */
	...
	{
	  fscanf (lrs_ifp, "%ld", &Q->dict_limit);
	  fprintf (lrs_ofp, "\n*cache %ld", Q->dict_limit);
	  if (Q->dict_limit < 1)
	    Q->dict_limit = 1;
	}
...
/* next block with volume rescaling must come before estimates are printed */
  ...
  if (hull)     /* output things that are specific to hull computation */
  ...
  else         /* output things specific to vertex/ray computation */
    {
      ...
      fprintf (lrs_ofp, " integer vertices=%ld ",count[4]);
      ...
      if ((cest[2] > 0) || (cest[0] > 0))
        {
	fprintf (lrs_ofp, "\n*Estimates: vertices=%g rays=%g", count[1]+cest[1], count[0]+cest[0]);
	fprintf (lrs_ofp, " bases=%g integer vertices=%g ",count[2]+cest[2], count[4]+cest[4]);
  ...
  fprintf (lrs_ofp, "\n*Dictionary Cache: max size= %ld misses= %ld/%ld   Tree Depth= %ld", Q->dict_count, Q->cache_misses, Q->cache_tries, Q->deepest);
...
pushQ (lrs_dat * global, long m, long d ,long m_A)
{
  ...
  if ((global->Qtail->next) == global->Qhead)
    {
      /* the Queue is full */
      if (global->dict_count < global->dict_limit)
	{
