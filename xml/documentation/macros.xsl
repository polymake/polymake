	...
	<div class="sidebar">
		<div class="sidebox">
          ...
          <!-- FIXME: remove the condition as soon as the doxygen documentation ripens enough to be published without disgrace -->
            	...
            	<xsl:if test="contains($version_text,'snapshot')">
					<li> <a href="http://polymake.mathematik.tu-darmstadt.de/release_docs/">release documentation</a></li>
            	</xsl:if>
			</ul>  
		</div>  
	</div>
...
<xsl:template mode="version" match="pm:commit">
  <xsl:text> extracted from git commit </xsl:text> <xsl:value-of select="normalize-space(current())"/>
</xsl:template>
